//
//  NewPatientPhoneViewController.swift
//  vivally
//
//  Created by Nadia Karina Camacho Cabrera on 1/10/23.
//

import UIKit
import PhoneNumberKit

class NewPatientPhoneViewController: BaseNavViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var suffixDropdown: DropdownOriginView!
    ...
    @IBOutlet weak var mostBothersomeDropdown: DropdownOriginView!
    ...
    override func viewDidLoad() {
        ...
        addPatientButton.backgroundColor = UIColor.casperBlue
        setupDemographics()
        ...
        addPatientButton.isHidden = editInfo
    ...
    func setupStacks(){
        ...
        title = editInfo ? "Edit Patient Information" : "New Patient Information"
        subjectView.isHidden = !editInfo
        
        firstNameStack.setup(title: "First name")
        middleNameStack.setup(title: "Middle name/initial (Optional)")
        lastNameStack.setup(title: "Last name")
        suffixDropdown.setup(title: "Suffix (Optional)", placeholder: "", options: suffixValues)
        ...
        mostBothersomeDropdown.setup(title: "Most bothersome symptom", options: demoDisplayNames[.mostBothersomeSymptom] ?? [])
        ...
        suffixDropdown.delegate = self
        studyDropdown.delegate = self
        therapyScheduleDropdown.delegate = self
        mostBothersomeDropdown.delegate = self
    ...
    func getPatientModel() -> PatientExists?{
        ...
        do{
            ...
            patientExists.suffix = suffixDropdown.contentLabel.text
    ...
    func loadClinics(){
        ...
        let soloStudy = studyValues.count < 2
        singleStudyID = soloStudy ? studyValues.first : nil
        studyDropdown.isHidden = soloStudy
        loadScheduleValues()
    }
    ...
    func loadValues(){
        ...
        garmentDropdown.selectRow(ind: garmentSelected.rawValue)
        ...
        if let suffixExists = patientData?.suffix{
            ...
            suffixDropdown.selectRow(ind: suffixSelected.rawValue)
        }
        ...
        if let botherSomeCode = patientData?.mostBothersomeSymptom{
            if let symptomsIds = demoIds[.mostBothersomeSymptom]{
                ...
                if foundMatch{
                    ...
                    mostBothersomeDropdown.selectRow(ind: row)
                }
            }
        }
    ...
    func preSelect(){
        if singleStudyID == nil{
            ...
            studyDropdown.selectRow(ind: studyRowSelected ?? 0)
        }
    ...
    func addPatientValidation()  {
        ...
        addPatientButton.isEnabled = allFieldsHaveText
        
        if !addPatientButton.isEnabled{
            addPatientButton.backgroundColor = UIColor.casperBlue
        }
    }
...
/*--*///  MARK: Extensions
extension NewPatientPhoneViewController: DropdownOriginViewDelegate{
    func dropSelected(ind: Int, option: String, sender: DropdownOriginView) {
        ...
        else if sender == mostBothersomeDropdown{
