...
//  vivally
//
//  Created by Nadia Karina Camacho Cabrera on 2/24/23.
//
...
class FirmwareUpdatePopupViewController: BasePopupViewController {
    ...
    func changedState(){
        ...
        case .firmwareFailed:
            titleLabel.text = "Update firmware failed"
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 3.0){
                    ...
                    if !BluetoothManager.sharedInstance.isConnectedToDevice(){
                        ...
                        for dev in discoveredDevices{
                            if dev.mac == OTAManager.sharedInstance.otaMACAddress{
...
extension FirmwareUpdatePopupViewController: BluetoothManagerDelegate{
    func didDiscoverDevice(discoveredDevice: DiscoveredDevice) {}
    
    func didConnectToDevice(device: CBPeripheral) {
        if OTAManager.sharedInstance.updateRunning && OTAManager.sharedInstance.progress != 100{
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0){
...
extension FirmwareUpdatePopupViewController: OTAManagerDelegate{
    func stateUpdated() {
