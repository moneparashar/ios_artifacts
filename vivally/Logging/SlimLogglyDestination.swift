...
class SlimLogglyDestination: LogDestination {
    func log<T>(_ message: @autoclosure () -> T, level: LogLevel, category: [LogCategories], filename: String, line: Int) {
        print("slim loggly called, remove")
    }
    ...
    fileprivate lazy var standardFields: [String: String] = {
        ...
        if let infodict = Bundle.main.infoDictionary {
            if let appname = infodict["CFBundleName"] as? String {
                dict["appname"] = "Avation.iOS"
            }
            if let appname = infodict["CFBundleVersion"] as? String {
                dict["appversion"] = appname
            }
        }
        var strId = KeychainManager.sharedInstance.accountData == nil ? 0 : KeychainManager.sharedInstance.accountData?.userModel?.id ?? 0
        dict["id"] = String(strId)
        ...
        dict["ApplicationName"] = "Vivally.iOS"
        dict["EnvironmentName"] = "Staging"         //change to Staging/Develop
        return dict
    }()
    ...
    private func updateID(){
        let strId = KeychainManager.sharedInstance.accountData == nil ? 0 : KeychainManager.sharedInstance.accountData?.userModel?.id ?? 0
        standardFields["id"] = String(strId)
    }
